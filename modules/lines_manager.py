...
class CrossLines:
    ...
    def cross_down_to_up(self): # Продажа
        _, sell_lines, pre_last_kline, pre_pre_last_kline = self.__get_data()
        
        for sell_line in sell_lines[::-1]:
            if pre_last_kline > sell_line: # Цена последней свечи выше линии
                if pre_pre_last_kline < sell_line: # Цена предпоследней свечи ниже чем линия продажи
                    if pre_last_kline > pre_pre_last_kline: # Цена последней свечи выше чем свечи до нее
                        print(sell_line)
                        return True
        return False
    
    def cross_up_to_down(self): # Покупка
        buy_lines, _, pre_last_kline, pre_pre_last_kline = self.__get_data()
        
        for buy_line in buy_lines:
            if pre_last_kline < buy_line: # Цена последней свечи ниже линии
                if pre_pre_last_kline > buy_line: # Цена предпоследней свечи выше чем линия продажи
